...
import { GameStatusContext, type GameStatus } from "./GameStatusContext";
...
export class StatusBarWidget {
  ...
  private autoPlayBg: Graphics;
  private autoPlayTxt: Text;
  ...
  constructor(ctx: GameStatusContext, layout: Layout, focusManager?: FocusManager) {
    ...
    this.playingTabTxt = new Text({ text: "Playing", style: new TextStyle({ fontFamily: "Arial", fontSize: 12, fill: 0x1a1a1a, fontWeight: "bold" }) });
    this.playingTabTxt.anchor.set(0.5, 0.5);
    playingTab.addChild(this.playingTabTxt);
    this._playingTab = playingTab;

    this.gameIdTxt = new Text({ text: "Current Game #—", style: new TextStyle({ fontFamily: "Arial", fontSize: layout.statusFontLg, fill: 0xffd700, fontWeight: "bold" }) });
    this.gameIdTxt.anchor.set(0, 0.5);
    this.container.addChild(this.gameIdTxt);

    this.timerTxt = new Text({ text: "Ready", style: new TextStyle({ fontFamily: "Arial", fontSize: layout.statusFontSm, fill: 0xaaccff }) });
    ...
    // AutoPlay button
    const autoPlayBtn = new Container();
    autoPlayBtn.eventMode = "static";
    autoPlayBtn.cursor = "pointer";
    this.autoPlayBg = new Graphics();
    autoPlayBtn.addChild(this.autoPlayBg);
    this.autoPlayTxt = new Text({ text: "⏩ Auto: OFF", style: new TextStyle({ fontFamily: "Arial", fontSize: layout.statusFontSm, fill: 0xffffff, fontWeight: "bold" }) });
    this.autoPlayTxt.anchor.set(0.5, 0.5);
    autoPlayBtn.addChild(this.autoPlayTxt);
    autoPlayBtn.on("pointerdown", () => { audioManager.playClick(); this.ctx.toggleAutoPlay(); });
    autoPlayBtn.on("pointerover", () => { this.autoPlayBg.alpha = 0.8; });
    autoPlayBtn.on("pointerout", () => { this.autoPlayBg.alpha = 1; });
    this.container.addChild(autoPlayBtn);
    this._autoPlayBtn = autoPlayBtn;
    ...
    this.muteTxt = new Text({ text: "🔊 Sound", style: new TextStyle({ fontFamily: "Arial", fontSize: layout.statusFontSm, fill: 0xffffff }) });
    ...
    // Register with FocusManager so TV remote can reach these buttons
    if (focusManager) {
      focusManager.addItem(autoPlayBtn, () => { audioManager.playClick(); this.ctx.toggleAutoPlay(); }, { group: "default", label: "Auto Play" });
      focusManager.addItem(muteBtn, () => { audioManager.toggle(); }, { group: "default", label: "Mute" });
    }
  }

  private _playingTab!: Container;
  private _autoPlayBtn!: Container;
  private _muteBtn!: Container;
  private _muteBtnW: number = 65;
  private _autoBtnW: number = 110;
  ...
  private applyLayout(layout: Layout): void {
    ...
    const autoBtnW = Math.round(Math.max(80, 110 * layout.uiScale));
    const muteBtnW = Math.round(Math.max(55, 65 * layout.uiScale));
    this._btnH = btnH;
    this._autoBtnW = autoBtnW;
    this._muteBtnW = muteBtnW;

    this._autoPlayBtn.x = pad * 2;
    this._autoPlayBtn.y = row2Y - btnH / 2;
    this.autoPlayBg.clear();
    this.autoPlayBg.roundRect(0, 0, autoBtnW, btnH, btnH / 2);
    this.autoPlayBg.fill({ color: 0x2a5298 });
    this.autoPlayTxt.style.fontSize = statusFontSm;
    this.autoPlayTxt.x = autoBtnW / 2;
    this.autoPlayTxt.y = btnH / 2;

    this._muteBtn.x = pad * 2 + autoBtnW + pad * 2;
  ...
  private update(s: GameStatus): void {
    ...
    const ap = s.autoPlay;
    this.autoPlayTxt.text = `⏩ Auto: ${ap ? "ON " : "OFF"}`;
    this.autoPlayBg.clear();
    this.autoPlayBg.roundRect(0, 0, this._autoBtnW, this._btnH, this._btnH / 2);
    this.autoPlayBg.fill({ color: ap ? 0x27ae60 : 0x2a5298 });
